...
if (FALSE) {
  ...
  #nn <- create.grnn()
  nnet = smooth(learn(train.scale, variable.column = 3), 0.1)
  nnet
  guess(nnet, t(train.scale[10,1:2]))
}

if (FALSE) {
  n <- 900; set.seed(123456); sigma <- 0.1
...
if (FALSE) {
  ...
  y.sim <- sim(train_x/20., train_y/20., train_x/20., 0.01)
  y.sim[1:10]
