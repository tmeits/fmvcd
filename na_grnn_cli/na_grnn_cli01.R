...
na.grnn.cli <- function(cli_dataset){
  ...
  if(sum(is.na(prec_vector)) != 0){
    prec_filled <- sapply(as.integer(na.grnn(prec_vector)), setZeroNegativeNumber)
  } else prec_filled <- prec_vector
  if(sum(is.na(temp_vector)) != 0){
    #temp_filled <- as.integer(na.spline(temp_vector))
    temp_filled <- as.integer(na.grnn(temp_vector))
...
if(FALSE){
  ...
  setwd(filled_path); getwd(); file_name <- "1969.cli"; file_name <- "2014.cli"
  ...
  #3.43         0.00         3.81
  system.time(cli_dataset_filled <- na.grnn.cli(cli_dataset))
